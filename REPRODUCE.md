...
**Pillow is pinned to 12.0.0**, the version the crops were produced under. Letterboxing resamples,
so a different version could move a few pixels at the padding boundary; 11.2.1 has been tested and
reproduces all 8811 crops pixel for pixel, other versions have not been tested. A moved pixel would
change no split and no rubric score, and step 2's `verify` reports it rather than hiding it. The
exact versions behind every file in `expected/` are in `requirements.lock`.
...
|---|---|---|
| direct conversion | 80 / 100 | **capped to the middle band**, usable only as a mild domain shift probe: B2 is zero (gap 0.046, below 0.05) and E1 is at half, both traced to the uniform composed background |
